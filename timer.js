...
(function (root) {
  'use strict';
  const AMBIENCE = ['fire', 'beach', 'ocean', 'none'];
  ...
  class BurrowTimer {
    constructor(raw) {
      ...
      s.chosen = integer(raw.chosen, 0, 0, 3);
      ...
      s.earned = Array.isArray(raw.earned) ? raw.earned.filter(n => Number.isInteger(n) && n >= 0 && n < 4).slice(-100) : [];
