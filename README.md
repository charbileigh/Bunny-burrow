...
- All thirteen WAV audio files are bundled and cached for offline playback.
...
3. Upload ALL files, including `sw.js`, `timer.js`, `mobile.js` and all thirteen WAVs.
...
The new service worker uses cache version `mobile-7-soft-bunnies`. An update waits
...
- `audio/` — all ten focus loops and three bell files.
...
All ten focus sound options can be previewed, selected, saved and played offline.
...
their new Biscuit/Pearl colours. All ten sounds and three bells remain included.
